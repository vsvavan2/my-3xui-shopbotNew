...
Сделан мод этого скрипта теперь есть 
Доступные Платёжки (Shop Bot)

- YooKassa — карточные оплаты; вебхук POST /yookassa-webhook .
- YooMoney (кошелёк, P2P) — вебхук POST /yoomoney-webhook ; поддерживает подпись SHA1 с yoomoney_secret .
- CryptoBot — крипто‑инвойсы Telegram; вебхук POST /cryptobot-webhook .
- Heleket — шлюз с подписью MD5; вебхук POST /heleket-webhook .
- TON (ончейн) — отслеживание входящих транзакций по TonAPI; вебхук POST /ton-webhook .
- Telegram Stars — начисление через Stars; включается флагом, работает через настройки без отдельного вебхука
...
```bash
curl -sSL https://raw.githubusercontent.com/vsvavan2/my-3xui-shopbotNew/main/install.sh | sudo bash
```
https://raw.githubusercontent.com/vsvavan2/my-3xui-shopbotNew/refs/heads/main/install.sh
...
И если захотите автора поддержите
